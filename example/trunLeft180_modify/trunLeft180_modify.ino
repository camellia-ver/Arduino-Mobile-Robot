...
int defaultPower = 100;  // 기본 주행(라인트레이싱) 속력 (80~120)
...
void simpleLineTracing()
{
  ...
  { // 양쪽 모두 검정색인 경우, 뒤로 돌기 (왼쪽으로 180도 회전)
    driveMotors(DIRECTION_FORWARD, 70, DIRECTION_FORWARD, 70); // 감속
    delay( 20 );
    stopMotors(); // 먼저 정지시킴
    delay( 100 );

    driveMotors(DIRECTION_BACKWARD, 70, DIRECTION_BACKWARD, 70); // 뒤로 후진
    ...
    driveMotors(DIRECTION_BACKWARD, TURN_POWER, DIRECTION_FORWARD, TURN_POWER);
    while( analogRead(IR_SENSOR_LEFT_PIN) < MIN_BLACK_THRESHOLD)

...
void  simpleLineTracing()
{
  ...
  { // 양쪽 IR센서 모두 검정색을 감지한 경우, (ex) 교차선
    ...
    while( analogRead(IR_SENSOR_LEFT_PIN) < MAX_WHITE_THRESHOLD ) // 흰색인 동안
      delay( 1 );
    delay( 40 );
    
    turnLeft( TURN_POWER ); // 2차 좌회전 진행 (반대쪽 흰바탕 도착)
    while( analogRead(IR_SENSOR_LEFT_PIN) > MIN_BLACK_THRESHOLD ) // 검은색인 동안
      delay( 1 );
    delay( 40 );
    ...
    moveForward( defaultPower ); // 좌회전 이후의 직진 시작
  }
